from sensor.exception import SensorException
from sensor.logger import logging
from sensor.entity.config_entity import ModelEvaluationConfig, ModelPusherConfig
from sensor.entity.artifact_entity import ModelPusherArtifact, ModelTrainerArtifact, ModelEvaluationArtifact
import os, sys
from sensor.ml.metric.classification_metric import get_classification_score
from sensor.utils.main_utils import save_object, load_object, write_yaml_file
import shutil

class ModelPusher:
    def __init__(
        self,
        model_pusher_config:ModelPusherConfig,
        model_eval_artifact: ModelEvaluationArtifact
    ):
        logging.info("initiating ModelPusher class")
        try:
            self.model_pusher_config= model_pusher_config
            self.model_eval_artifact= model_eval_artifact
        except Exception as e:
            raise SensorException(e,sys)

    def initiate_model_pusher(self)->ModelPusherArtifact:
        try:
            logging.info("Entering the initiate_model_pusher method of ModelPusher class")

            trained_model_path= self.model_pusher_config.model_file_path

            logging.infor("creating the Model_Pusher_Dir to save the model in initiate_model_pusher")

            model_file_path= self.model_pusher_config.model_file_path
            os.makedirs(os.path.dirname(saved_model_path), exists_ok=True)
            shutil.copy(src=trained_model_file_path, dst= saved_model_path)

            logging.info("preparing the artifact for Initiate_model_pushr")

            model_pusher_artifact= ModelPusherArtifact(saved_model_path=saved_model_path. model_file_path=model_file_path)
            return model_pusher_artifact

        except Exception as e:
            raise SensorException(e,sys)
